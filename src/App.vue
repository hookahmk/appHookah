...
<template>
  <section class="flex relative">
    ...
    <div v-if="store.popUp" class="fixed right-4 top-40 rounded-xl z-50 p-3 bg-amber-300 text-[1.1rem] items-center flex">
      Item Adicionado ao carrinho
      <FaceSmileIcon class="h-6 text-yellow-100" />
    </div>
    ...
    <div class="w-full 0 relative bg-gradient-to-b via-90%">
      ...
      <header class="fixed w-full z-50 bg-black h-24 flex justify-center">
        <nav class="flex items-center gap-11">
          ...
          <div class="md:flex gap-11 hidden text-white">
            ...
            <RouterLink to="/acendedor" class="link">Acendedor</RouterLink>
            <RouterLink to="/pegador" class="link">Pegador</RouterLink>
            <RouterLink to="/rosh" class="link">Rosh</RouterLink>
          </div>

          <div class="p-3 absolute right-0 rounded-md flex justify-center items-center">
            <div class="mr-4 flex">
              <ShoppingCartIcon class="h-10 text-green-800 cursor-pointer"
        ...
        <Transition>
          <div v-if="modal"
            class="flex flex-col h-screen min-w-44 z-50 right-0 fixed bg-black opacity-80 top-0 gap-4 border-b-2 text-white text-[1.3rem] mt-2 px-3">
            <XMarkIcon @click="modal = false" class="h-10 cursor-pointer text-white font-bold" />
            <RouterLink @click="modal = false" to="/"  class="link">Home</RouterLink>
            <RouterLink @click="modal = false" to="/essencia"  class="link">Essências</RouterLink>
            <RouterLink @click="modal = false" to="/aluminio"  class="link">Aluminio</RouterLink>
            <RouterLink @click="modal = false" to="/carvao"  class="link">Carvão</RouterLink>
            <RouterLink @click="modal = false" to="/acendedor" class="link">Acendedor</RouterLink>
            <RouterLink @click="modal = false" to="/pegador" class="link">Pegador</RouterLink>
            <RouterLink @click="modal = false" to="/rosh" class="link">Rosh</RouterLink>
          </div>
        </Transition>
      ...
      <div class="mt-44 border-2">
        <RouterView />
        <a class="fixed bottom-3 right-5"
